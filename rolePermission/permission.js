...
export const getUnitHierarchy = async function getUnitHierarchy(
  parentId,
  ...
  try {
    ...
    const subResultsPromises = subUserIds.map((userId) =>
      getUnitHierarchy(userId, processedIds)
    );
...
export const getCategoryHierarchy = async function getCategoryHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getCategoryHierarchy(userId, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getCompanyDetailHierarchy =
  async function getCompanyDetailHierarchy(parentId, processedIds = new Set()) {
    try {
      if (processedIds.has(parentId)) {
        return [];
      }
      ...
      const subUserIds = users.map((user) => user._id);
      const subResultsPromises = subUserIds.map((userId) =>
        getCompanyDetailHierarchy(userId, processedIds)
      );
      const subResults = await Promise.all(subResultsPromises);
      return results.concat(subResults.flat());
    } catch (error) {
...
export const getWarehouseHierarchy = async function getWarehouseHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getWarehouseHierarchy(userId, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getProductHierarchy = async function getProductHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getProductHierarchy(userId, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getTargetCreationHierarchy =
  async function getTargetCreationHierarchy(
    parentId,
    processedIds = new Set()
  ) {
    try {
      ...
      const subUserIds = users.map((user) => user._id);
      const subResultsPromises = subUserIds.map((userId) =>
        getTargetCreationHierarchy(userId, processedIds)
      );
      const subResults = await Promise.all(subResultsPromises);
      return results.concat(subResults.flat());
    } catch (error) {
...
export const getPromotionHierarchy = async function getPromotionHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    for (const promotion of promotions) {
      subResultsPromises.push(
        getPromotionHierarchy(promotion._id, processedIds)
      );
    }
    const subResults = await Promise.all(subResultsPromises);
...
export const getUserHierarchy = async function getUserHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getUserHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
...
export const getUserHierarchyBottomToTop =
  async function getUserHierarchyBottomToTop(userId, processedIds = new Set()) {
    try {
      ...
      const parentResults = await getUserHierarchyBottomToTop(
        createdByUserId,
        processedIds
      );
...
export const getCustomerHierarchy = async function getCustomerHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getCustomerHierarchy(userId, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getStockHierarchy = async function getUnitHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
...
export const getUserWarehouseHierarchy = async function getUserHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    processedIds.add(parentId);
    const users = await User.find({ created_by: parentId })
      .populate({ path: "rolename", model: "role" })
      .populate({ path: "created_by", model: "user" })
      .populate({ path: "productItems.productId", model: "product" })
      .lean();
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getUserHierarchy(userId, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return users.concat(subResults.flat());
  } catch (error) {
...
export const getUserHierarchyWithProducts =
  async function getUserHierarchyWithProducts(
    parentId,
    processedIds = new Set()
  ) {
    try {
      ...
      return {
        ...
        products: products.concat(
          subResults.map((result) => result.products).flat()
        ),
      };
    } catch (error) {
...
// ---------------------------------------------------------------------
export const getCreditNoteHierarchy = async function getCustomerHierarchy(
  parentId,
  processedIds = new Set()
) {
  try {
    ...
    const [users, creditNote] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" })
        .populate({ path: "created_by", model: "user" })
        .lean(),
      CreditNote.find({ created_by: parentId, status: "Active" })
        .populate({ path: "created_by", model: "user" })
...
// --------------------------------------------------
export const getCompanyDetailHierarchy1 =
  async function getCompanyDetailHierarchy(
    userId,
    ff,
    processedIds = new Set()
  ) {
    try {
      ...
      const subResultsPromises = subUserIds.map((subUserId) =>
        getCompanyDetailHierarchy(subUserId, processedIds)
      );
      const subResults = await Promise.all(subResultsPromises);
      if (parentUser && parentUser.created_by) {
        const parentResults = await getCompanyDetailHierarchy(
          parentUser.created_by,
          processedIds
        );
...
export const getCompanyDetailHierarchyfirst =
  async function getCompanyDetailHierarchy(
    userId,
    isTopToBottom = true,
    ff,
      ...
      if (isTopToBottom) {
        ...
        subUserResults = await Promise.all(
          subUsers.map((subUser) =>
            getCompanyDetailHierarchy(subUser._id, isTopToBottom, processedIds)
          )
        );
      } else {
        ...
        if (parentUser && parentUser.created_by) {
          parentResults = await getCompanyDetailHierarchy(
            parentUser.created_by,
            isTopToBottom,
            processedIds
          );
...
export const getCompanyDetailHierarchyTopToBottom =
  async function getCompanyDetailHierarchyTopToBottom(
    userId,
    processedIds = new Set()
  ) {
    try {
      ...
      const subUserResults = await Promise.all(
        subUsers.map((subUser) =>
          getCompanyDetailHierarchyTopToBottom(subUser._id, processedIds)
        )
      );
...
export const getCompanyDetailHierarchyBottomToTop =
  async function getCompanyDetailHierarchyBottomToTop(
    userId,
    processedIds = new Set()
  ) {
    try {
      ...
      const parentResults =
        parentUser && parentUser.created_by
          ? await getCompanyDetailHierarchyBottomToTop(
              parentUser.created_by,
              processedIds
            )
          : [];
