...
export const getUserHierarchyDetails = async function getUserHierarchyDetails(
  parentId,
  database
) {
  try {
...
export const getUserHierarchyBottomToTop =
  async function getUserHierarchyBottomToTop(
    userId,
    database,
    processedIds = new Set()
  ) {
  ...
  try {
    ...
    const [users, customers] = await Promise.all([
      ...
      Customer.find({
        ...
        .populate({ path: "rolename", model: "role" })
        .populate({ path: "assignTransporter._id", model: "transporter" })
        .lean(),
    ]);
    let results = customers;
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getCustomerHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
...
//-----------------------------------------------------------------------------------
export const getOrderHierarchy = async function getOrderHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getOrderHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getSalesReturnHierarchy = async function getSalesReturnHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const [users, customers] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" }).lean(),
      SalesReturn.find({ userId: parentId })
        .populate({
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getSalesReturnHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getCreateOrderHierarchy = async function getCreateOrderHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const [users, customers] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" }).lean(),
      CreateOrder.find({ userId: parentId, status: { $ne: "Deactive" } })
        .populate({
          path: "orderItems.productId",
          model: "product",
        })
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getCreateOrderHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getGoodDispatchHierarchy = async function getGoodDispatchHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const [users, customers] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" }).lean(),
      GoodDispatch.find({ userId: parentId })
        .sort({ sortorder: -1 })
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getGoodDispatchHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getCreditNoteHierarchy = async function getCreditNoteHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const [users, customers] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" }).lean(),
      CreditNote.find({ userId: parentId })
        .sort({ sortorder: -1 })
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getCreditNoteHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
...
export const getDebitNoteHierarchy = async function getDebitNoteHierarchy(
  parentId,
  database,
  processedIds = new Set()
) {
  try {
    ...
    const [users, customers] = await Promise.all([
      User.find({ created_by: parentId, status: "Active" }).lean(),
      DebitNote.find({ userId: parentId })
        .sort({ sortorder: -1 })
        .populate({ path: "productItems.productId", model: "product" })
        .populate({ path: "userId", model: "user" })
        .populate({ path: "partyId", model: "customer" })
    ...
    const subUserIds = users.map((user) => user._id);
    const subResultsPromises = subUserIds.map((userId) =>
      getDebitNoteHierarchy(userId, database, processedIds)
    );
    const subResults = await Promise.all(subResultsPromises);
    return results.concat(subResults.flat());
  } catch (error) {
  ...
  async function getTargetCreationHierarchy(
    ...
    try {
      ...
      const [users, targetCreations] = await Promise.all([
        User.find({ created_by: parentId, status: "Active" }).lean(),
        TargetCreation.find({
          $or: [{ created_by: parentId }, { salesPersonId: parentId }],
        })
...
export const getPurchaseOrderHierarchy =
  async function getPurchaseOrderHierarchy(
    parentId,
    database,
    processedIds = new Set()
  ) {
    try {
      ...
      const [users, customers] = await Promise.all([
        User.find({ created_by: parentId, status: "Active" }).lean(),
        PurchaseOrder.find({ userId: parentId })
          .sort({ sortorder: -1 })
          .populate({ path: "orderItems.productId", model: "product" })
          .exec(),
      ]);
      ...
      const subUserIds = users.map((user) => user._id);
      const subResultsPromises = subUserIds.map((userId) =>
        getPurchaseOrderHierarchy(userId, database, processedIds)
      );
      const subResults = await Promise.all(subResultsPromises);
      return results.concat(subResults.flat());
    } catch (error) {
...
export const getPurchaseReturnHierarchy =
  async function getPurchaseReturnHierarchy(
    parentId,
    database,
    processedIds = new Set()
  ) {
    try {
      ...
      const [users, customers] = await Promise.all([
        User.find({ created_by: parentId, status: "Active" }).lean(),
        PurchaseReturn.find({ created_by: parentId })
          .populate({
            path: "returnItems.productId",
            model: "product",
          })
      ...
      const subUserIds = users.map((user) => user._id);
      const subResultsPromises = subUserIds.map((userId) =>
        getPurchaseReturnHierarchy(userId, database, processedIds)
      );
      const subResults = await Promise.all(subResultsPromises);
      return results.concat(subResults.flat());
    } catch (error) {
...
export const getCustomerHierarchyDeadParty =
