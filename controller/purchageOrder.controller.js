...
import {
  ...
  addProductInWarehouse2,
  addProductInWarehouse3,
  ...
  try {
    ...
    if (!user) {
      ...
      if (date1.toDateString() === date2.toDateString()) {
        ...
        return res
          .status(400)
          .json({ message: "can not purchaseOrder of next date" });
      }
    }
  } catch (err) {
    console.log(err);
...
export const Purch = async (req, res, next) => {
  try {
    ...
    console.log(
      `Querying stock for warehouseId: ${req.params.id} with date range: ${startOfDay} to ${endOfDay}`
    );
    ...
    if (existingStock) {
      ...
      return res
        .status(404)
        .json({ message: "Product not found in stock", status: false });
    }

    await stock.save();
    return res
      .status(200)
      .json({ message: "Stock updated successfully", status: true });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
