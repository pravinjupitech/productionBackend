...
export const updateOrderStatusByDeliveryBoy = async (req, res) => {
  try {
        ...
        if (product) {
          ...
          if (warehouse) {
            ...
            if (pro) {
              ...
              console.error(
                `Product Not Found In Warehouse Product ID ${orderItem.productId}`
              );
            }
          } else {
    ...
    return res
      .status(200)
      .json({ message: "Delivery Status Updated Successfully", status: true });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
export const OrderCancelWarehouse = async (req, res, next) => {
  try {
    ...
    if (!productFound) {
      return res
        .status(404)
        .json({ message: "Product Not Found in Order", status: false });
    }
