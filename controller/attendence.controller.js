...
export const saveAttendence = async (req, res, next) => {
  try {
    ...
    return face
      ? res.status(200).json({ message: "successfully", status: true })
      : res
          .status(400)
...
export const saveImage = async (req, res, next) => {
  try {
    ...
    if (!pan) {
      return res
        .status(200)
        .json({ message: "pan no. not found", status: false });
    }
...
export const UserRegister = async (req, res, next) => {
  try {
    const response = await axios.post(
      "http://13.201.119.216:8050/api/register",
      req.body
    );
...
export const UserRecognition = async (req, res, next) => {
  try {
    const response = await axios.post(
      "http://13.201.119.216:8050/api/recognize",
      req.body
    );
    if (response.data.status) {
      ...
      return res
        .status(400)
        .json({ message: response.data.message, status: false });
    }
  } catch (err) {
    ...
    return res
      ...
      .json({
        ...
        status: false,
        successCode: err.response.status,
      });
  }
};
export const Attendance = async (req, res, next) => {
  try {
    const attend = await axios.get(
      `http://13.201.119.216:8050/api/attendanceAws/${req.params.database}`
    );
    if (attend.data.status) {
      return res
        .status(200)
        .json({ Attendance: attend.data.Attendance, status: true });
    }
    return res
      .status(200)
      .json({ message: "something went wrong", status: false });
...
export const CreateCollection = async (req, res, next) => {
  try {
    const response = await axios.post(
      "http://13.201.119.216:8050/api/createcollection",
      req.body
    );
    if (response.status === 200) {
      return res
        .status(200)
        .json({ message: response.data.message, status: true });
    }
...
export const AttendanceListById = async (req, res, next) => {
  try {
    const attend = await axios.get(
      `http://13.201.119.216:8050/api/attendanceAwsById/${req.params.id}`
    );
    if (attend.data.status) {
      return res
        .status(200)
...
export const AttendanceList = async (req, res, next) => {
  try {
    const attend = await axios.get(
      `http://13.201.119.216:8050/api/localDataAttendance/${req.params.database}`
    );
    if (attend.data.status) {
      return res
        .status(200)
