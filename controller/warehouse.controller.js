...
export const SignIn = async (req, res, next) => {
  try {
    ...
    if (!warehouse) {
      return res
        .status(400)
        .json({ message: "Incorrect Warehouse Username", status: false });
    }
...
export const SaveWarehouse = async (req, res, next) => {
  try {
    if (req.body.id) {
      ...
      return res
        .status(400)
        .json({ message: "warehouse id required", status: false });
    }
...
export const ViewWarehouseById = async (req, res, next) => {
  try {
    let warehouse = await Warehouse.findById({ _id: req.params.id })
      .sort({ sortorder: -1 })
      .populate({ path: "productItems.productId", model: "rowProduct" }); //raw product for did change
    return warehouse
      ? res.status(200).json({ Warehouse: warehouse, status: true })
      : res.status(404).json({ error: "Not Found", status: false });
  } catch (err) {
...
export const ViewWarehouseByIdRowProduct = async (req, res, next) => {
  try {
    let warehouse = await Warehouse.findById({ _id: req.params.id })
      .sort({ sortorder: -1 })
...
export const UpdateWarehouse = async (req, res, next) => {
  try {
    ...
    if (!existingWarehouse) {
      return res
        .status(404)
        .json({ error: "warehouse not found", status: false });
    } else {
      const updatedWarehouse = req.body;
      await Warehouse.findByIdAndUpdate(warehouseId, updatedWarehouse, {
        new: true,
      });
      return res
        .status(200)
        .json({ message: "Warehouse Updated Successfully", status: true });
    }
  } catch (err) {
    console.error(err);
    return res
...
export const getWarehouseData = async (req, res, next) => {
  try {
    const factory = await Factory.find({ warehouseToId: req.params.id })
      .populate({
...
export const WarehouseReport = async (req, res, next) => {
  try {
    ...
    let warehouseList = [];
    const warehouse = await Warehouse.find(targetQuery)
      .sort({ sortorder: -1 })
      .populate({ path: "productItems.productId", model: "product" })
      .populate({ path: "productItems.productId", model: "product" });
    ...
    return warehouseList.length > 0
      ? res.status(200).json({ Warehouse: warehouseList, status: true })
...
export const warehouseProductItem = async (body, lastStock) => {
  if (
    ...
    !Array.isArray(lastStock.productItems)
  ) {
...
export const WarehouseDifferenceReport = async (req, res, next) => {
  try {
    ...
    let productList = [];
    let stockList = [];
    const warehouse = await Warehouse.find(targetQuery).populate({
      path: "productItems.productId",
      model: "product",
    });
    ...
    const stockDate = req.body.endDate
      ? new Date(req.body.endDate)
      : currentDate;
    const lastStock = await Stock.find({
      database: req.params.database,
      createdAt: { $gte: stockDate },
    }).populate({ path: "productItems.productId", model: "product" });
...
export const warehouseProductItem1 = async (body, lastStock) => {
  ...
  for (let id of body) {
    ...
    if (stock) {
      ...
      matchingStocks[id.productId._id].damageItem +=
        id?.damageItem?.transferQty || 0;
    } else {
      if (!matchingStocks[id.productId._id]) {
        matchingStocks[id.productId._id] = {
          productId: id.productId,
...
export const HSNStockSummary = async (req, res, next) => {
  try {
    ...
    let orders = [];
    const salesOrder = await Warehouse.find(targetQuery).populate({
      path: "productItems.productId",
      model: "product",
    });
...
export const StockCalculate11 = async (req, res, next) => {
  try {
    ...
    const product = await Product.find({ database: req.params.database }).sort({
      sortorder: -1,
    });
    ...
    WarehouseStock.DamageStock =
      WarehouseStock.DamageStock == NaN ? 0 : WarehouseStock.DamageStock;
...
export const StockCalculate = async (req, res, next) => {
  try {
    ...
    WarehouseStock.DamageStock = isNaN(WarehouseStock.DamageStock)
      ? 0
      : WarehouseStock.DamageStock;
    WarehouseStock.DeadStock = await ViewOverDueStock(req.params.database);
    ...
    for (let item of exist) {
      ...
      if (warehouse) {
        let ware = {
          ...
          oBAmount:
            (item.Opening_Stock * item.Purchase_Rate * 100) /
            (item.GSTRate + 100),
...
export const StockClose = async () => {
  try {
    const warehouses = await Warehouse.find({ status: "Active" }).sort({
      sortorder: -1,
    });
    if (warehouses.length === 0) {
